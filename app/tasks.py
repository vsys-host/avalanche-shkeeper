...
def make_multipayout(symbol, payout_list, fee):
    ...
    elif symbol in config['TOKENS'][config["CURRENT_AVALANCHE_NETWORK"]].keys():
...
def refresh_balances():
    ...
    try:
        ...
        for account in list_acccounts:
            ...
            for token in config['TOKENS'][config["CURRENT_AVALANCHE_NETWORK"]].keys():
            ...
            if have_tokens in config['TOKENS'][config["CURRENT_AVALANCHE_NETWORK"]].keys():
